...
- Coil 3
- Paging 3
- Android Keystore (管理加密密钥) 与本地持久化存储
- Kotlin Coroutines
- Navigation 3
...
### v0.2：Technical Validation / OAuth

- [x] 核验 Pinterest 官方 OAuth Authorization Code flow。
- [x] 核验 Redirect URI 精确匹配、Scope 最小权限和 State 回调要求。
- [x] 核验 Access Token、continuous Refresh Token 和刷新请求机制。
- [x] 核验 User、Boards、Pins、分页、Rate Limit、错误码和开发者政策资料。
- [x] 记录官方未说明的 PKCE、原生回调、client secret 和图片资源问题。
- [x] 纯 Wear OS 客户端的 client secret 方案【已决策】。
- [ ] 确认 OAuth 应用注册、回调方式和权限范围。
- [ ] 确认 Token 生命周期、刷新策略和安全存储策略。
- [ ] 核对应用创建日期及对应 Refresh Token 模式。
- [ ] 逐个核验 MVP endpoint 的完整字段、Scope、分页和错误码。
- [ ] 完成 OAuth 方案评审后，才进入 Kotlin 实现。

### Sprint 1: Project Skeleton

- [x] 建立包结构 (core, data, domain, presentation)。
- [x] 建立 Domain Model (OAuthToken, UserAccount, Board, Pin)。
- [x] 建立 Repository 接口 (Auth, User, Board, Pin)。
- [x] 建立 API DTOs (OAuthToken, User, Board, Pin)。
- [x] 建立 Mapper (Auth, User, Board, Pin)。
- [x] 遵守 Architecture Freeze，无业务逻辑。

### Sprint 1.5: Project Cleanup

- [x] 迁移包名至 `com.wear.pin`。
...
- [x] 核验 Pinterest 官方 OAuth/API 文档和 Developer Guidelines。
...
- Pinterest 官方认证文档没有说明 PKCE 是否要求或支持。
- 纯 Wear OS 客户端将 client secret 保存在本地，接受风险【已决策】。
- 图片 URL/CDN 直接访问和临时缓存政策作为开放问题（Open Issue），不阻塞 MVP。
...
- PKCE 是否要求或支持。
- Token 生命周期、刷新机制和 Token 密钥加密持久化存储实现细节。
