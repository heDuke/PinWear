...
## Technical Validation 状态

v0.2 Technical Validation 已完成第一轮 Pinterest 官方文档核验，确认了 Authorization Code flow、Redirect URI 精确匹配、Scope 最小权限、Access Token/Refresh Token 基本机制、分页参数、Rate Limit 类别和主要开发者政策边界。

以下事项仍不能从已核验的官方文档中确认：

- PKCE 是否要求或支持。
- 纯 Wear OS 原生客户端的 Redirect URI 方案。
- Pinterest 应用创建日期对应的 Refresh Token 模式。
- 所有 MVP endpoint 的完整字段、Scope、分页和错误码。

以下为【已决策】事项：
- client secret 放客户端：采用纯 Wear OS 客户端方案，client secret 及 Token 保存在本地，接受相关风险。不再作为阻塞项。

以下为开放问题（Open Issue），不作为 MVP 阻塞项：
- 图片 URL、CDN 访问和临时/内存缓存规则。

以上【待确认】事项完成前，不得开始对应的 Kotlin 实现。详细核验记录见 [API.md](API.md)。
